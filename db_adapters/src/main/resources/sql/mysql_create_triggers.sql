DROP TRIGGER IF EXISTS `server`.`%table%_after_insert`;;
DROP TRIGGER IF EXISTS `server`.`%table%_after_update`;;
DROP TRIGGER IF EXISTS `server`.`%table%_after_delete`;;
CREATE TRIGGER `server`.`%table%_after_insert` AFTER INSERT ON `%table%`
    FOR EACH ROW
        ...
        END IF;;
CREATE TRIGGER `server`.`%table%_after_update` AFTER UPDATE ON `%table%`
    FOR EACH ROW
        IF (SELECT USER() NOT LIKE '%syncuser%@%') THEN
            UPDATE %table%%md_suffix% SET %flag_md% = 1 WHERE %pk_md% = NEW.%pk_data%;
        END IF;;
CREATE TRIGGER `server`.`%table%_after_delete` AFTER DELETE ON `%table%`
    FOR EACH ROW
