...
public class SyncTestImpl {
    ...
    static {
        ...
        logConfigurator.setFileName(
            Environment.getExternalStorageDirectory() + File.separator + "doubleganger_test.log");
        logConfigurator.setRootLevel(Level.DEBUG);
        // Set log level of a specific logger
        logConfigurator.setLevel("doubleganger_test", Level.ALL);
        logConfigurator.configure();
        LOG = LoggerFactory.getLogger("doubleganger_test");
    }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.GINGERBREAD
                ...
                CONF.setClientDatabaseAdapter(
                    de.consistec.doubleganger.android.adapter.GingerbreadSQLiteDatabaseAdapter.class);

            } else if (Build.VERSION.SDK_INT >= 14) {
                // ICS
                in = assetManager.open("ics.properties");
                CONF.setClientDatabaseAdapter(
                    de.consistec.doubleganger.android.adapter.ICSSQLiteDatabaseAdapter.class);
            } else {
