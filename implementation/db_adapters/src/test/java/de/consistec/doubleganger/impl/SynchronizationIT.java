...
public class SynchronizationIT {
    ...
    @Test
    public void testServerMysqlOffClientMysqlOff() throws SyncException, ContextException, SQLException, IOException {
        init(MYSQL, false, MYSQL, false);
        testWithDataBeforeFirstSync();
    }


    @Test
    public void testServerMysqlOnClientMysqlOn() throws SyncException, ContextException, SQLException, IOException {
        init(MYSQL, true, MYSQL, true);
        testWithDataBeforeFirstSync();
    }

    @Test
    public void testServerPostgresOffClientPostgresOff() throws SyncException, ContextException, SQLException,
        IOException {
        init(POSTGRESQL, false, POSTGRESQL, false);
        testWithDataBeforeFirstSync();
    }

    @Test
    public void testServerPostgresOnClientPostgresOn() throws SyncException, ContextException, SQLException,
        IOException {
        init(POSTGRESQL, true, POSTGRESQL, true);
        testWithDataBeforeFirstSync();
    }

    @Test
    public void testServerSqliteOffClientSqliteOff() throws SyncException, ContextException, SQLException, IOException {
        init(SQLITE, false, SQLITE, false);
        testWithDataBeforeFirstSync();
    }

    @Test
    public void testServerMysqlOnClientPostgresOff() throws SyncException, ContextException, SQLException, IOException {
        init(MYSQL, true, POSTGRESQL, false);
        testWithDataBeforeFirstSync();
    }

    @Ignore("This test behaves strangely. Please re-check the SQLite magic!")
    @Test
    public void testServerPostgresOnClientSqliteOff() throws SyncException, ContextException, SQLException,
        IOException {
        init(POSTGRESQL, true, SQLITE, false);
        testWithDataBeforeFirstSync();
    }

    @Ignore("This test behaves strangely. Please re-check the SQLite magic!")
    @Test
    public void testServerSqliteOffClientMysqlOff() throws SyncException, ContextException, SQLException, IOException {
        init(SQLITE, false, MYSQL, false);
        testWithDataBeforeFirstSync();
    }

    public void init(DummyDataSource.SupportedDatabases serverDbType, boolean serverTriggers,
                     DummyDataSource.SupportedDatabases clientDbType, boolean clientTriggers
    ) throws SyncException, ContextException,
        SQLException, IOException {

        clientDb = new TestDatabase(serverDbType, CLIENT, clientTriggers);
        clientDb.init();
        clientDb.dropTables(tableNames);

        serverDb = new TestDatabase(clientDbType, SERVER, serverTriggers);
        serverDb.init();
    ...
    public void testWithDataBeforeFirstSync() throws SQLException, ContextException, SyncException {
        ...
        TestScenario.UserInformation userInfo = scenario.getUserInformation();
        ...
        try {
            ...
            scenario.synchronize(tableNames,
                getServerConflictResolver(userInfo.getUserAction(), userInfo.getSelectedChange()), BIDIRECTIONAL);
            ...
            scenario.synchronize(tableNames,
                getServerConflictResolver(userInfo.getUserAction(), userInfo.getSelectedChange()));
    ...
    public IConflictListener getServerConflictResolver(final UserDecision userAction, final UserDecision selectedChange
    ) {
        return new IConflictListener() {
            @Override
            public ResolvedChange resolve(Map<String, Object> serverData, Map<String, Object> clientData) {
                ResolvedChange resolvedChange = new ResolvedChange(userAction);
                TestScenario.UserInformation userInfo = scenario.getUserInformation();

                if (userInfo.getUserAction() == UserDecision.SERVER_CHANGE || userInfo.getSelectedChange() == UserDecision.EDIT_SERVER_CHANGE) {
                    resolvedChange.setRowData(serverData);
                } else {
                    resolvedChange.setRowData(clientData);
                }

                resolvedChange.setSelectedDecision(selectedChange);
...
//            @Override
//            public ResolvedChange resolve(Map<String, Object> serverData, Map<String, Object> clientData) {
//                ResolvedChange resolvedChange = new ResolvedChange(UserDecision.CLIENT_CHANGE);
//                resolvedChange.setRowData(clientData);
//                return resolvedChange;
//            }
//        };
    ...
    public static Collection<Object[]> AllScenarii() {
        ...
        return Arrays.asList(new Object[][]{
            ...
            {new TestScenario("ServerMod ClientDel", BIDIRECTIONAL, FIRE_EVENT)
                .addStep(SERVER, updateRow2b)
                .addStep(CLIENT, deleteRow2)
                .addUserDecision(UserDecision.USER_EDIT, UserDecision.EDIT_SERVER_CHANGE)
                .expectServer("SS")
                .expectClient("CS")},
            {new TestScenario("ServerMod ClientDel", BIDIRECTIONAL, FIRE_EVENT)
                .addStep(SERVER, updateRow2b)
                .addStep(CLIENT, deleteRow2)
                .addUserDecision(UserDecision.SERVER_CHANGE, null)
                .expectServer("SS")
                .expectClient("CS")},
            {new TestScenario("ServerMod ClientDel", BIDIRECTIONAL, FIRE_EVENT)
                .addStep(SERVER, updateRow2b)
                .addStep(CLIENT, deleteRow2)
                .addUserDecision(UserDecision.CLIENT_CHANGE, null)
                .expectServer("S")
                .expectClient("C")},
            ...
            {new TestScenario("ServerDel ClientMod", BIDIRECTIONAL, FIRE_EVENT)
                .addStep(SERVER, deleteRow2)
                .addStep(CLIENT, updateRow2b)
                .addUserDecision(UserDecision.USER_EDIT, UserDecision.EDIT_CLIENT_CHANGE)
                .expectServer("SC")
                .expectClient("CC")},
            {new TestScenario("ServerDel ClientMod", BIDIRECTIONAL, FIRE_EVENT)
                .addStep(SERVER, deleteRow2)
            ...
            {new TestScenario("ServerDel ClientDel", BIDIRECTIONAL, FIRE_EVENT)
                ...
                .expectServer("S")
                .expectClient("C")},
            {new TestScenario("* ServerDel ClientDel invalid", SERVER_TO_CLIENT,
                CLIENT_WINS)
                .addStep(SERVER, deleteRow2)
                .addStep(CLIENT, deleteRow2)
                .expectException(IllegalStateException.class, NOT_SUPPORTED_CONFLICT_STRATEGY)},
            {new TestScenario("* ServerDel ClientDel invalid", CLIENT_TO_SERVER,
                SERVER_WINS)
                .addStep(SERVER, deleteRow2)
                .addStep(CLIENT, deleteRow2)
                .expectException(IllegalStateException.class, NOT_SUPPORTED_CONFLICT_STRATEGY)}
